...
_DATA_PATH = Path(__file__).parent / "data" / "jst_japan.json"


class JSTJapanSource(BaseSource):
    id = "jst_japan"
    ...
    def _load(self):
        ...
        if not _DATA_PATH.exists():
            logger.warning("JST: data file NOT FOUND at %s", _DATA_PATH)
            self._records = []
        else:
            with open(_DATA_PATH, encoding="utf-8") as f:
                self._records = json.load(f)
            logger.warning("JST: loaded %d records from %s", len(self._records), _DATA_PATH)
        self._loaded = True
    ...
    def _to_technology(self, rec: dict) -> Technology:
        return Technology(
            ...
            org_name=rec.get("institute", "JST"),
