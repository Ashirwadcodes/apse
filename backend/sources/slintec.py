...
_DATA_PATH = Path(__file__).parent / "data" / "slintec.json"


class SLINTECSource(BaseSource):
    id = "slintec"
    name = "SLINTEC Sri Lanka"
    country = "Sri Lanka"
    institution = "Sri Lanka Institute of Nanotechnology"
    status = "Metadata search"
    url = "https://www.slintec.lk/what-we-offer/ready-to-go-technologies/"
    ttl_seconds = 86400
    ...
    def _load(self):
        ...
        if not _DATA_PATH.exists():
            logger.warning("SLINTEC: data file NOT FOUND at %s", _DATA_PATH)
            self._records = []
        else:
            with open(_DATA_PATH, encoding="utf-8-sig") as f:
                self._records = json.load(f)
            logger.info("SLINTEC: loaded %d records from %s", len(self._records), _DATA_PATH)
        self._loaded = True
    ...
    def _to_technology(self, rec: dict) -> Technology:
        return Technology(
            ...
            org_name=rec.get("institute", "SLINTEC"),
